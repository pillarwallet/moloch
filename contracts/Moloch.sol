...
contract Moloch is ReentrancyGuard {
    ...
    *****************/
    function submitProposal(
        ...
        IERC20 theTributeToken = IERC20(tributeToken);
        require(SafeERC20.safeTransferFrom(theTributeToken, msg.sender, address(this), tributeOffered), "tribute token transfer failed");
    ...
    function sponsorProposal(uint256 proposalId) public nonReentrant onlyDelegate {
        ...
        require(SafeERC20.safeTransferFrom(depositToken, msg.sender, address(this), proposalDeposit), "proposal deposit token transfer failed");
    ...
    function processProposal(uint256 proposalIndex) public nonReentrant {
        ...
        if (didPass) {
            ...
            require(
                SafeERC20.safeTransfer(proposal.tributeToken, address(guildBank), proposal.tributeOffered),
        ...
        // PROPOSAL FAILED
            ...
            if (!emergencyProcessing) {
                require(
                    SafeERC20.safeTransfer(proposal.tributeToken, proposal.proposer, proposal.tributeOffered),
    ...
    function _returnDeposit(address sponsor) internal {
        require(
            SafeERC20.safeTransfer(depositToken, msg.sender, processingReward),
            "failed to send processing reward to msg.sender"
        );

        require(
            SafeERC20.safeTransfer(depositToken, sponsor, proposalDeposit.sub(processingReward)),
